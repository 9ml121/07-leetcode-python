...
# todo 解法1：利用二进制& and运算(推荐！！)
def method2():
    # x = input().split('.')
    # y = input().split('.')
    # z = input().split('.')
    x = '255.255.255.0'.split('.')
    y = '192.168.0.254'.split('.')
    z = '192.168.0.1'.split('.')
...
def method1():
    ...
    # step3：判断是否是同一子网络
    def check_net(mask: str, ip1: str, ip2: str):
        ...
        for i in range(4):
            ...
            l22 = []
            for j in range(8):
                l11.append(str(int(l0[i][j]) and int(l1[i][j])))
                l22.append(str(int(l0[i][j]) and int(l2[i][j])))
            s11 = ''.join(l11)
            s22 = ''.join(l22)
            if s11 != s22:
                return 2
