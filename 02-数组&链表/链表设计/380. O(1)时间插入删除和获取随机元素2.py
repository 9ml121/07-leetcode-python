...
class RandomizedSet2:
    ...
    第一版：用python中set实现，不能满足getRandom的需求
    """
    ...
    def getRandom(self) -> int:
        if len(self.hashSet) > 0:
            # python中的 random.choice 函数从列表中随机选取一个元素的时间复杂度是O(1)
            # todo  但是, 将set转换为list的操作是一个O(n)操作!!
...
class TestRandomizedSet:
    ...
    def test_insert(self):
        assert self.rs.insert(3) == True
        assert self.rs.insert(3) == False
        assert self.rs.getRandom() == 3
        assert self.rs.getRandom() == 3
        assert self.rs.insert(1) == True
        assert self.rs.remove(3) == True
        assert self.rs.insert(0) == True
        assert self.rs.remove(0) == True
